...
#pragma omp for schedule(runtime) reduction(+ : sum_f_i, sum_f)
	...
	// Open file to check if it's empty
...
#endif
	{
		...
		if (F != NULL)
		{
			...
			if (length == 0)
			{
				fprintf(F, "MPI,OpenMP,Schedule,Barycenter,Elapsed Time,Number of Processes,Number of Threads,Iterate Times,Number of Fishes\n");
			}
			// Write the data

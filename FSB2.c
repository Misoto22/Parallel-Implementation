...
#pragma omp for schedule(static)
	...
	if ((m == m1) && (num_fishes <= max_items)) {
		...
		max_items = INT_MAX / 2;
		offset = 0LL;
		do { //This loop is for large amounts of data
			if (rank == 0) { // rank 0, using rank 1 for self exchange 
...
#pragma omp for schedule(static)
	...
	bari = 0.0;
	t1 = get_wtime();
	for (t = 0; t < num_iterate_steps; t++) { //Every fish's moving coordinate depend on previous coordinate, so this loop can't parallel.
		sum_f = 0.0;
		sum_f_i = 0.0;
		max_delt_f = -1.0;
		if (t == 0) {
...
#pragma omp for schedule(runtime) private(tmp_d) reduction(max:max_delt_f)
				...
				for (i = 0; i < mx; i++) { //Move
					do {
						...
						if (fish[i].x < MIN_XY) { //check for left limit
							fish[i].x = MIN_XY;
						}
						else  if (fish[i].x > MAX_XY) { //check for right limit
							fish[i].x = MAX_XY;
						}
						fish[i].y += generate_range_random_double(MIN_MOVING_XY, MAX_MOVING_XY);
						if (fish[i].y < MIN_XY) { //check for bottom limit
							fish[i].y = MIN_XY;
						}
						else  if (fish[i].y > MAX_XY) { //check for top limit
							fish[i].y = MAX_XY;
						}
...
#pragma omp for schedule(runtime) reduction(+:sum_f_i, sum_f)
				...
				for (i = 0; i < mx; i++) { //Eat
					...
					else if (fishext[i].w > MAX_WEIGHT) { //check for over weight
